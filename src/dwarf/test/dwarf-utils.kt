...
val test0 = """
/var/folders/56/ldwtkqhx2t5g06z1s3tt9gth0000gp/T/dwarfdump_test_complex8372771032510584574/poisoned.kexe.dSYM/Contents/Resources/DWARF/poisoned.kexe:	file format Mach-O 64-bit x86-64
0x00000000: Compile Unit: length = 0x000109ba version = 0x0002 abbr_offset = 0x0000 addr_size = 0x08 (next unit at 0x000109be)

0x000108d1: DW_TAG_subprogram
              DW_AT_low_pc	(0x0000000100053f50)
              DW_AT_high_pc	(0x0000000100054058)
              DW_AT_frame_base	(DW_OP_reg6 RBP)
              DW_AT_MIPS_linkage_name	("kfun:main${'$'}<anonymous>_1#internal")
              DW_AT_name	("main${'$'}<anonymous>_1")
              DW_AT_decl_file	("/var/folders/56/ldwtkqhx2t5g06z1s3tt9gth0000gp/T/dwarfdump_test_complex8372771032510584574/poison.kt")
              DW_AT_decl_line	(8)
              DW_AT_type	(0x00000032 "kotlin.Int")
              DW_AT_external	(0x01)
    ...
    fun parse(reader: Reader) {
        reader.forEachLine { line ->
            tagRegexp.find(line)?.apply {
                val str = this.destructured.component2().strip()
                if (str == "NULL")
                    return@forEachLine
                tag(DwarfTag.by(str))
                return@forEachLine
            ...
            currentAttributePayload.appendln(line)
        }
    }
}

fun main() {
    val parser = DwarfUtilParser()
    //val fileReader = FileReader("/Users/minamoto/ws/.git-trees/mpetrov/debug/programm.dump")
    parser.parse(StringReader(test0))

    val tags = parser.tags.filterIsInstance(DwarfTagSubprogram::class.java).filter { it.path != null }
    tags.forEach {
        println("${it.name} @${it.path}")
    }

    parser.tags.flatMap { it.attributes.values }.filter { it.attribute == DwarfAttribute.Attribute.DW_AT_decl_file }.forEach {
        println(it.rvString)
    }
    println("end")
}